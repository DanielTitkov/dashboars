...
func (iq *ItemQuery) QueryTaskInstance() *TaskInstanceQuery {
	query := &TaskInstanceQuery{config: iq.config}
...
// the "task_instance" edge. The optional arguments are used to configure the query builder of the edge.
...
func (iq *ItemQuery) sqlAll(ctx context.Context) ([]*Item, error) {
	...
	if query := iq.withTaskInstance; query != nil {
		...
		for i := range nodes {
			if nodes[i].task_instance_items == nil {
				continue
			}
			fk := *nodes[i].task_instance_items
			if _, ok := nodeids[fk]; !ok {
				ids = append(ids, fk)
			}
			nodeids[fk] = append(nodeids[fk], nodes[i])
		}
		query.Where(taskinstance.IDIn(ids...))
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "task_instance_items" returned %v`, n.ID)
			}
			for i := range nodes {
